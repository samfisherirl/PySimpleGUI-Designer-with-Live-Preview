...
class TreeNode:
    ...
    def move_tree_node(self, direction):
        if self.parent is None:
            ...
            if direction == "back":
                container[position], container[position - 1] = container[
                    position - 1], container[position]
            else:
                next_pos = position + 1 if position + 1 != len(
                    container) else 0
                # if next_pos == len(container):
                # 	next_pos = 0
                container[position], container[next_pos] = container[
                    next_pos], container[position]
    ...
    def get_layout(self):
        ...
        if self.container:
            ...
            return self.element(
                *[
                    self.necessary_args[x]
                    if x != "layout" else self.container[0].get_layout()
                    for x in self.necessary_args_name
                ], **self.optional_args)
        ...
        return self.element(
            *[self.necessary_args[x] for x in self.necessary_args_name],
            **self.optional_args)
        ...
        for parsed_row in parsed_layout:
            ...
            for parsed_elem in parsed_row:
                ...
                for i in range(len(elem.necessary_args_name)):
                    elem.necessary_args[
                        elem.necessary_args_name[i]] = parsed_elem[1][i]
                necessary_to_optional = parsed_elem[1][
                    len(elem.necessary_args_name):]
                if necessary_to_optional:
                    for i in range(len(necessary_to_optional)):
                        elem.optional_args[elem.default_optional_args_name[
                            i]] = necessary_to_optional[i]
                for key in parsed_elem[2]:
                    elem.optional_args[key] = parsed_elem[2][key]
                if GUI_CLASSES[parsed_elem[0]] in CONTAINER_ELEMENTS:
                    elem.container = [
                        TreeNode.make_tree_from_parsed_layout(
                            elem.necessary_args["layout"])
                    ]
    ...
    def layout_to_string(self):
        ...
        if self.element in ("Root", "Row"):
            string_layout = "[{}]".format(", ".join(
                [x.layout_to_string() for x in self.container]))
        else:
            args = [
                TreeNode.convert_to_str(self.necessary_args[arg_name]) if
                self.element not in CONTAINER_ELEMENTS or arg_name != "layout"
                else self.container[0].layout_to_string()
                for arg_name in self.necessary_args_name
            ]

            kargs = [
                "{0}={1}".format(arg_name, TreeNode.convert_to_str(arg_value))
                for arg_name, arg_value in self.optional_args.items()
                if self.optional_args[arg_name] !=
                self.default_optional_args[arg_name]
            ]
    ...
    def get_properties_layout(self, elem_size, property_count):
        ...
        layout.append([sg.Text(text="Use \"quotes\" and commas! ")])
        layout.append([sg.Text(text="Just as you would in Python")])
            ...
            layout.append([
                sg.Text(text=arg_name, 
                        tooltip=(self.args_docs[arg_name]
                                 if arg_name in self.args_docs else None)),
                sg.Input(default_text=value, size=('111', '1'),
        ...
        for arg_name, arg_value in self.optional_args.items():
            ...
            layout.append([
                sg.Text(text=arg_name,
                        tooltip=(self.args_docs[arg_name]
                                 if arg_name in self.args_docs else None)),
                sg.Input(default_text=value, size=('100', '1'),
                         key=str(property_count) + "_" + arg_name)
            ])
